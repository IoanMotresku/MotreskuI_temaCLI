...
namespace Tema_6
{
    public class Window : GameWindow
    {
        ...
        protected override void OnUpdateFrame(FrameEventArgs e)
        {
            ...
            if (keyboard[Key.L] && !lastKeyboard[Key.L])
                camera.toogleLook();

            if (keyboard[Key.F] && !lastKeyboard[Key.F])
                Cube.StartFalling();

            if (keyboard[Key.R] && !lastKeyboard[Key.R])
                Cube.StopFalling();
        ...
        private void DisplayHelp()
        {
            ...
            Console.WriteLine(" L - Schimbare pozitie camera");
            Console.WriteLine(" F - Incepe cadere");
            Console.WriteLine(" R - Opreste modul de cadere");
